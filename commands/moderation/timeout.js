...
module.exports = {
  ...
  async execute(message, args) {
    ...
    if (!target) return err("❌ Usage: `,timeout @user <duration> [reason]`\n**Duration examples:** `10m` `1h` `2d`");

    const durationStr = args[1];
    if (!durationStr) return err("❌ Please provide a duration. Examples: `10m` `1h` `2d`");

    // Parse duration string → ms
    const match = durationStr.match(/^(\d+)(s|m|h|d)$/i);
    if (!match) return err("❌ Invalid duration format. Use `10m`, `2h`, `1d` etc.");

    const units = { s: 1000, m: 60000, h: 3600000, d: 86400000 };
    const ms = parseInt(match[1]) * units[match[2].toLowerCase()];

    const MAX_MS = 28 * 24 * 60 * 60 * 1000; // 28 days Discord limit
    if (ms > MAX_MS) return err("❌ Maximum timeout duration is 28 days.");
    if (ms < 5000)   return err("❌ Minimum timeout duration is 5 seconds.");

    const reason = args.slice(2).join(" ") || "No reason provided";

    if (!target.moderatable) return err("❌ I can't timeout this user (check my role position).");
